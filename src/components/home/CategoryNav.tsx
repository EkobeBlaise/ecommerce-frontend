...
export const CategoryNav: React.FC<CategoryNavProps> = ({
  ...
  const [categorySlug, setCategorySlug] = useState(propCategorySlug || '');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (categoryId) {
        category = allCategories.find((c: Category) => c.id === categoryId);
      } else {
        category = allCategories.find((c: Category) => c.gender === gender);
      }

      if (category) {
        setCategorySlug(propCategorySlug || category.slug);
        const allGroups = category.categoryGroups || [];
        const filteredGroups = allGroups.filter(
          (g: CategoryGroup) => g.categoryId === category.id && g.isActive !== false
        );
  ...
  const getLink = (group: CategoryGroup, sub?: SubCategory): string => {
    const slug = categorySlug || propCategorySlug || '';
    if (sub) {
      return `/${slug}/${group.slug}/${sub.slug}`;
    }
  ...
  const getMegaMenuImage = (groupName: string): string => {
    const images: Record<string, string> = {
      'NEW IN': 'https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=400&h=300&fit=crop',
      Clothing: 'https://images.unsplash.com/photo-1483985988355-763728e1935b?w=400&h=300&fit=crop',
  ...
  return (
    <div className="relative w-full z-40 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
          ...
          {groups.map((group) => {
            ...
            return (
              ...
              >
                <Link
                  to={getLink(group)}
                  className="flex items-center gap-1 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-pink-600 whitespace-nowrap py-2 transition"
                >
      ...
      {/* Full‑width dropdown with absolute positioning – no shift, no overlap */}
      {activeCategory && (
        <div
          ...
          <div className="container mx-auto px-4 py-6">
            <div className="grid grid-cols-4 gap-8">
              <div className="col-span-3 grid grid-cols-3 gap-6">
                {(() => {
                  // ✅ Find the active group by name, then get its subcategories by ID
                      ...
                      <div className="space-y-2">
                        {firstCol.map((sub) => (
                          <Link
                            ...
                            className="block text-sm text-gray-600 dark:text-gray-400 hover:text-pink-600 transition"
