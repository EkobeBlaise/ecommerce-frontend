...
export const KidsCategoryNav: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const kidsCategory = allCategories.find((c: Category) => c.gender === 'kids');
      console.log('🧒 Kids category found:', kidsCategory);
      
      if (kidsCategory) {
        const kidsGroups = await categoryManagementNewService.getCategoryGroups(kidsCategory.id);
        setGroups(kidsGroups);
        
        const subsMap: Record<string, SubCategory[]> = {};
        for (const group of kidsGroups) {
        ...
        console.log('🧒 Kids groups loaded:', kidsGroups.map(g => g.name));
      } else {
        console.warn('⚠️ No kids\' category found');
        setGroups([]);
        setSubCategories({});
      }
    } catch (error) {
      console.error('Error loading kids\' category data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getLink = (group: CategoryGroup, sub?: SubCategory): string => {
    if (sub) {
      ...
      return `/kids/${group.slug}/${subSlug}`;
    }
    return `/kids/${group.slug}`;
  };
  ...
  const getMegaMenuImage = (groupName: string): string => {
    const images: Record<string, string> = {
      'NEW IN': 'https://images.unsplash.com/photo-1519457431-44ccd64d5790?w=400&h=300&fit=crop',
      'Girls': 'https://images.unsplash.com/photo-1519457431-44ccd64d5790?w=400&h=300&fit=crop',
      'Boys': 'https://images.unsplash.com/photo-1503917988258-0d06b4e1bc17?w=400&h=300&fit=crop',
      'Baby': 'https://images.unsplash.com/photo-1515459969601-3e6c63e3a6d8?w=400&h=300&fit=crop',
      'Shoes': 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&h=300&fit=crop',
      'Accessories': 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?w=400&h=300&fit=crop',
      'Designer': 'https://images.unsplash.com/photo-1445205170230-053b83016050?w=400&h=300&fit=crop',
    ...
    return images[groupName] || 'https://images.unsplash.com/photo-1519457431-44ccd64d5790?w=400&h=300&fit=crop';
  };

  const getMegaMenuImageAlt = (groupName: string): string => {
    return `Kids ${groupName} collection`;
  };

  const getMegaMenuImageGradient = (groupName: string): string => {
    const gradients: Record<string, string> = {
      'NEW IN': 'from-yellow-100 to-orange-100',
      'Girls': 'from-pink-100 to-rose-100',
      'Boys': 'from-blue-100 to-cyan-100',
      'Baby': 'from-pink-100 to-purple-100',
      'Shoes': 'from-green-100 to-teal-100',
      'Accessories': 'from-yellow-100 to-orange-100',
      'Designer': 'from-purple-100 to-indigo-100',
      'Sports': 'from-red-100 to-orange-100',
      'Brands': 'from-indigo-100 to-purple-100',
      'Sale': 'from-red-100 to-pink-100',
    };
    return gradients[groupName] || 'from-pink-100 to-purple-100';
  };
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-2 text-sm text-gray-500">Loading kids' categories...</span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
        <div className="container mx-auto px-4 py-3">
          <p className="text-sm text-gray-500 dark:text-gray-400">No kids' categories available</p>
        </div>
      </div>
    );
  }
          ...
          {groups.map((group) => {
            ...
            return (
              ...
              >
                <Link
                  to={getLink(group)}
                  className="flex items-center gap-1 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-green-600 whitespace-nowrap py-2 transition"
                >
                      ...
                      <div className="space-y-2">
                        {firstCol.map((sub) => (
                          <Link
                            ...
                            className="block text-sm text-gray-600 dark:text-gray-400 hover:text-green-600 transition"
                  ...
                  <img
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src =
                        'https://images.unsplash.com/photo-1519457431-44ccd64d5790?w=400&h=300&fit=crop';
                    }}
                  />
...
export default KidsCategoryNav;