...
const MensFashion: React.FC = () => {
  ...
  const heroSlides = [
    {
      image: "https://images.unsplash.com/photo-1617137968427-85924c800c5e?w=1920&h=500&fit=crop",
      title: "Pack light, look great:",
      subtitle: "vacation styles for beach days, city strolls and outdoor getaways",
      cta: "Shop now",
      link: "/collections/men-vacation"
    },
    {
      image: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=1920&h=500&fit=crop",
      title: "Summer Essentials",
      subtitle: "Discover the latest trends for the sunny season",
      cta: "Explore now",
      link: "/collections/men-summer"
    },
    {
      image: "https://images.unsplash.com/photo-1487222477894-8943e31ef7b2?w=1920&h=500&fit=crop",
      title: "Designer Edit",
      subtitle: "Luxury pieces for the modern man",
      cta: "Shop designer",
      link: "/collections/men-designer"
    }
  ];

  const stories = [
    { title: "The Perfect Sneaker Guide", brand: "NIKE", description: "How to style the hottest sneakers of the season", image: "https://images.unsplash.com/photo-1600185365483-26d7a4cc7519?w=400&h=300&fit=crop" },
    { title: "Summer Style Essentials", brand: "ADIDAS", description: "Must-have pieces for your summer wardrobe", image: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=400&h=300&fit=crop" }
  ];
  ...
  const loadProducts = async () => {
    try {
      ...
      const mensProducts = productsArray.filter(p => {
        if (!p) return false;
        if (p.gender) {
        ...
        if (p.category) {
          ...
          return categoryLower.includes('men') || 
                 categoryLower.includes('male') ||
                 categoryLower.includes('mens') ||
                 categoryLower.includes("men's");
        }
        return false;
      });
      
      setProducts(mensProducts.length > 0 ? mensProducts : productsArray.slice(0, 12));
      
      if (mensProducts.length > 0) {
        await loadBrandFeatures(mensProducts);
      }
    } catch (error) {
  ...
  const getFallbackBrandFeatures = (productList: Product[]) => {
    return [
      { name: "BOSS", tagline: "Tailored for success", image: "https://images.unsplash.com/photo-1617137968427-85924c800c5e?w=400&h=500&fit=crop", products: productList.slice(0, 2) },
      { name: "adidas", tagline: "Original Handball Spezial", image: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=400&h=500&fit=crop", products: productList.slice(2, 4) },
      { name: "HUGO", tagline: "Modern tailoring", image: "https://images.unsplash.com/photo-1487222477894-8943e31ef7b2?w=400&h=500&fit=crop", products: productList.slice(4, 6) },
      { name: "Tommy Hilfiger", tagline: "American classics", image: "https://images.unsplash.com/photo-1617137968427-85924c800c5e?w=400&h=500&fit=crop", products: productList.slice(6, 8) },
      { name: "Calvin Klein", tagline: "Modern minimalism", image: "https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=400&h=500&fit=crop", products: productList.slice(8, 10) }
    ];
  };
  ...
  const loadMoreData = async () => {
    try {
      ...
      const menCategory = allCategories.find((c: any) => c.gender === 'men' || c.slug === 'men');
      
      if (menCategory) {
        const allSubCategories: string[] = [];
        const groups = menCategory.categoryGroups || [];
      ...
      setMoreBrands([
        ...
        "BOSS", "Hugo", "Tommy Hilfiger", "Calvin Klein", "Ralph Lauren", "Lacoste",
        "Levi's", "Diesel", "Carhartt", "The North Face", "Patagonia", "Columbia",
        "Timberland", "Clarks", "Dr. Martens", "New Balance", "Asics", "Reebok"
      ]);
      setMoreInspiration([
        "T-Shirts", "Polo Shirts", "Dress Shirts", "Jeans", "Chinos", "Joggers",
        "Blazers", "Suits", "Trench Coats", "Leather Jackets", "Bomber Jackets", "Hoodies",
        "Sneakers", "Boots", "Loafers", "Oxford Shoes", "Sandals", "Trainers",
        "Watches", "Sunglasses", "Belts", "Bags", "Wallets", "Caps"
      ]);
    }
  };
  ...
  // Helper to render a product section with men's products only
  ...
  return (
    <>
      <SEO
        title="Men's Fashion"
        description="Discover the latest men's fashion trends. Shop stylish clothing, shoes, and accessories for every occasion."
        keywords={['men fashion', 'men clothing', 'shirts', 'shoes', 'accessories']}
      />
      ...
      <div className="min-h-screen bg-white dark:bg-gray-900">
        ...
        {/* Trending in Men's Fashion */}
        {renderProductSection(
          'Trending in Men\'s Fashion',
          'Most popular men\'s items this week',
        ...
        {renderProductSection(
          'New Arrivals',
          'Latest men\'s styles just added',
        ...
        {/* Men's Categories Grid (keep as is) */}
        <div className="container mx-auto px-4 py-8">
          <h2 className="text-xl font-bold mb-4 dark:text-white">Shop by Category</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link to="/men/clothing" className="relative overflow-hidden rounded-xl group">
              <img src="https://images.unsplash.com/photo-1556905055-8f358a7a47b2?w=400&h=300&fit=crop" alt="Clothing" className="w-full h-40 object-cover group-hover:scale-110 transition duration-500" />
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <span className="text-white font-bold text-lg">Clothing</span>
              </div>
            </Link>
            <Link to="/men/shoes" className="relative overflow-hidden rounded-xl group">
              <img src="https://images.unsplash.com/photo-1600185365483-26d7a4cc7519?w=400&h=300&fit=crop" alt="Shoes" className="w-full h-40 object-cover group-hover:scale-110 transition duration-500" />
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <span className="text-white font-bold text-lg">Shoes</span>
              </div>
            </Link>
            <Link to="/men/accessories" className="relative overflow-hidden rounded-xl group">
            ...
            <Link to="/men/sportswear" className="relative overflow-hidden rounded-xl group">
              <img src="https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=400&h=300&fit=crop" alt="Sportswear" className="w-full h-40 object-cover group-hover:scale-110 transition duration-500" />
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <span className="text-white font-bold text-lg">Sportswear</span>
              </div>
            </Link>
          </div>
        </div>
